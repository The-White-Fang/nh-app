	...
	return (
		<FlatList
			...
			renderItem={({ item }) => (
				<TouchableOpacity 
					style={styles.card} 
					onPress={() => router.push(`/(drawer)/sauce/lists/${item.id}`)}
					activeOpacity={0.8}
				>
					...
					<View style={styles.cardInfo}>
						...
						{item.description ? (
							<RegularText style={styles.cardDesc} numberOfLines={2}>{item.description}</RegularText>
						) : null}
...
function SauceListsDataWrapper({ setModalVisible, setIsCreating, setErrorMsg, newListName, newListDesc }: any) {
	const queryClient = useQueryClient();
...
export default function SauceLists() {
	...
	const { mutate: doCreateList, isPending: isCreating } = useMutation({
		mutationFn: () => createSauceList(newListName, newListDesc),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: ['sauce_lists'] });
		...
		<ProtectedScreen>
			<Screen safe_area={true} style={styles.root}>
				<View style={styles.header}>
					...
					<RegularText style={styles.headerTitle}>My Sauce Lists</RegularText>
				</View>

				<Suspense fallback={<SauceListsSkeleton />}>
					<SauceListsDataWrapper 
				...
				<Portal>
					<Modal visible={isModalVisible} onDismiss={() => setModalVisible(false)} contentContainerStyle={styles.modalContent}>
						...
						<View style={styles.modalActions}>
							<Button onPress={() => setModalVisible(false)} textColor={tw_colors.zinc300}>Cancel</Button>
