...
import { fetchLists, createList, togglePin, fetchListDetail } from '@/services/lists';
	...
	return (
		<FlatList
			...
			renderItem={({ item }) => (
				<TouchableOpacity 
					style={styles.card} 
					onPress={() => router.push(`/(drawer)/anime/lists/${item.id}`)}
					activeOpacity={0.8}
				>
...
function AnimeListsDataWrapper({ setModalVisible, setIsCreating, setErrorMsg, newListName, newListDesc }: any) {
	...
	return <AnimeListsContent pinToggle={pinToggle} lists={lists} />;
}
